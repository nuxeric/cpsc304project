CREATE TABLE personnel ( id INTEGER, birth_date DATE, first_name TEXT, last_name TEXT, PRIMARY KEY (id)
);
CREATE TABLE line_worker ( id INTEGER, responsibilities TEXT, PRIMARY KEY (id),
   FOREIGN KEY (id)
...
id INTEGER, number_of_employees_managed INTEGER, PRIMARY KEY (id),
  FOREIGN KEY (id)
  REFERENCES Personnel (id)
       ON DELETE CASCADE
       ON UPDATE CASCADE
...
CREATE TABLE region( postal_code CHAR(6), municipality TEXT, province_or_territory TEXT, PRIMARY KEY (postal_code)
);
CREATE TABLE difference_cache( larger DOUBLE PRECISION,
smaller DOUBLE PRECISION,
difference INTEGER NOT NULL, PRIMARY KEY (larger, smaller)
);

 CREATE TABLE warehouse (
id INTEGER,
total_volume DOUBLE PRECISION, occupied_volume DOUBLE PRECISION, street_address TEXT,
postal_code CHAR(6), PRIMARY KEY (id), FOREIGN KEY (postal_code)
REFERENCES region(postal_code) ON UPDATE CASCADE
ON DELETE RESTRICT,
FOREIGN KEY (total_volume, occupied_volume) REFERENCES difference_cache (larger, smaller) ON DELETE RESTRICT
ON UPDATE RESTRICT
);
...
id INTEGER, street_address TEXT, postal_code CHAR(6), PRIMARY KEY (id), FOREIGN KEY (postal_code)
REFERENCES region (postal_code) ON DELETE RESTRICT
ON UPDATE CASCADE
);
...
PRIMARY KEY (tag_name, inventory_type_name), FOREIGN KEY (tag_name)
       REFERENCES tag (name)
       ON DELETE CASCADE
       ON UPDATE CASCADE,
FOREIGN KEY (inventory_type_name) REFERENCES inventory_type (type_name) ON DELETE CASCADE
ON UPDATE CASCADE
);
...
CREATE TABLE delivers_inventory( serial_num INTEGER,
delivery_id INTEGER,
PRIMARY KEY (serial_num, delivery_id), FOREIGN KEY (serial_num)
REFERENCES inventory(serial_num) ON DELETE CASCADE
ON UPDATE CASCADE,
FOREIGN KEY (delivery_id) REFERENCES delivery (id) ON DELETE CASCADE
ON UPDATE CASCADE
);
CREATE TABLE production_facility ( id INTEGER,
producer_id INTEGER NOT NULL, postal_code CHAR (6), street_address TEXT,
PRIMARY KEY (id), FOREIGN KEY (postal_code)
REFERENCES region (postal_code) ON DELETE RESTRICT
ON UPDATE CASCADE,
...
id INTEGER,
producer_id INTEGER NOT NULL, date DATE,
description TEXT, PRIMARY KEY (id), FOREIGN KEY (producer_id)
       REFERENCES producer (id)
       ON DELETE CASCADE
       ON UPDATE CASCADE
);

CREATE TABLE recipient ( id INTEGER, street_address TEXT, postal_code CHAR(6), PRIMARY KEY (id), FOREIGN KEY (postal_code)
REFERENCES region (postal_code) ON DELETE RESTRICT
ON UPDATE CASCADE
);
...
PRIMARY KEY (delivery_id, recipient_id, warehouse_id), FOREIGN KEY (delivery_id)
...
producer_id INTEGER,
warehouse_id INTEGER,
PRIMARY KEY (delivery_id, producer_id, warehouse_id), FOREIGN KEY (delivery_id)
       REFERENCES delivery (id)
       ON DELETE CASCADE
       ON UPDATE CASCADE,
FOREIGN KEY (producer_id) REFERENCES recipient (id) ON DELETE CASCADE
ON UPDATE CASCADE,
FOREIGN KEY (warehouse_id) REFERENCES warehouse (id) ON DELETE CASCADE
...
pid INTEGER,
warehouse_id INTEGER,
PRIMARY KEY (pid, warehouse_id), FOREIGN KEY (pid)
       REFERENCES personnel (id)
       ON DELETE CASCADE
       ON UPDATE CASCADE,
FOREIGN KEY (warehouse_id) REFERENCES warehouse (id) ON DELETE CASCADE
